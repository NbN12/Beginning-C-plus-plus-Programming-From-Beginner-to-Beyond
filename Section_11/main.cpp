...
void display_menu();
void change_Duplicate_flag(bool& isDuplicate);
char get_selection();
...
int main() {
    ...
    do {
		...
		selection = get_selection(); // get selection return char with uppercase
        ...
        // Add a number to numbers
            ...
            add_element_to_list(numbers, isDuplicate);
        } 
        ...
        // Turn on/off allowing duplicate entries(default is on)
            ...
            change_Duplicate_flag(isDuplicate);
        }
        else if (selection != 'Q') {
...
void add_element_to_list(vector<int>& numbers, bool isDuplicate)
{
    int number;
...
void print_smallest_number(const vector<int>& numbers)
{
    if (numbers.size() == 0)
    {
...
void print_largest_number(const vector<int>& numbers)
{
    if (numbers.size() == 0)
    {
...
void search_number(const vector<int>& numbers)
{
    if (numbers.size() == 0)
    {
